...
<section class="content">
	...
	<div class="row">
        <div class="col-xs-12">
                <div class="box">
                    <div class="box-header with-border" style="display: flex; flex-direction:column;">
                        <div class="box-title">Mengelola Soal <span id="judul-tambah-soal"></span></div>
                        <button onclick="showForm(event)" data-status="1" class="btn btn-primary" style="width: 150px; margin-top: 14px;">Soal yang sudah ada</button>
                    </div><!-- /.box-header -->

                    <?php echo form_open('welcome/login','id="form-tambah-soal-by-topik" class="form-horizontal" style="display: none;"')?>
                        ...
                        <div class="box-header with-border">
                            ...
                            <div class="box-body">
                                ...
                                <div class="form-group">
                                    <label class="col-sm-3 control-label">Tambah Soal Dari Topik Lain</label>
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-2 control-label">Topik Soal</label>
                                    <div class="col-sm-10">
                                        <select class="form-control input-sm" onchange="topik_soal(event)" id="topik-soal" name="topik-soal">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-control input-sm" id="soal" name="soal">
                                            <option value="" selected>Soal Tidak Ada</option>
                                        </select>
                                    </div>
                                </div>
                                
                                <div class="form-group">
                                    <label class="col-sm-2 control-label">Set Soal</label>
                                    <div class="col-sm-4">
                                        <select class="form-control input-sm" id="tambah-set-soal" name="tambah-set-soal">
                        ...
                        <table id="table-soal" class="table table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th>Jawaban Benar</th>
                                    <th>Jawaban Salah</th>
                                    <th style="text-align: center;">Kesulitan</th>
                                    <th style="text-align: center;">Analisa</th>
                                    <th>Jawaban</th>
                                    <th></th>
                                </tr>
                            </thead>
...
<script lang="javascript">
    function showForm(item){
        const soalSudahAda = document.getElementById("form-tambah-soal-by-topik");
        const soalBaru = document.getElementById("form-tambah");
        let status = item.target.dataset.status

        // Soal Sudah Ada == 1
        // Soal Baru == 0

        if(status == 1){
            soalBaru.style.display = "none";
            soalSudahAda.style.display = "block";
            item.target.innerHTML = "Soal baru";
            item.target.dataset.status = "0"
        } else {
            soalBaru.style.display = "block";
            soalSudahAda.style.display = "none";
            item.target.innerHTML = "Soal yang sudah ada";
            item.target.dataset.status = "1"
        }

    }

    function topik_soal(item){
        const setData = item.target.value 
        const soal = document.getElementById("soal");

        let toHTML = "";

        if(setData == ""){
            toHTML += "<option value='' selected>Soal Tidak Ada</option>";
            soal.innerHTML = toHTML;
        } else {
            $.ajax({
                url:'<?php echo site_url().'/'.$url; ?>/soal_from_another_topik',
                type:"POST",
                data: {
                    "tambah-topik": setData
                },
                cache: false,
                success:function(respon){
                    ...
                    if(obj.length == 0){
                        ...
                        obj.forEach( (result) => {
                            toHTML += `<option value="${result.id}">${result.soal}</option>`
                        } )
                        soal.innerHTML = toHTML;
                    }
    ...
    $('#form-tambah-soal-by-topik').submit(function(e){
            // e.preventDefault();
            
            const form = $(this);

            const topik = form.find('select[name="topik-soal"]').val();
            const soal = form.find('select[name="soal"]').val();
            const set_soal = form.find('select[name="tambah-set-soal"]').val();

            if($("#topik").val() == topik){
                alert("Topik soal tidak boleh sama dengan topik yang ingin di masukan");
            } else {
                $("#modal-proses").modal('show');
                $.ajax({
                    url:"<?php echo site_url().'/'.$url; ?>/tambah_soal_by_topik",
                    type:"POST",
                    data: {
                        "topik-id": $("#topik").val(),
                        "soal-id": soal,
                        "tambah-set-soal": set_soal
                    },
                    cache: false,
                    success:function(respon){
                        var obj = $.parseJSON(respon);
