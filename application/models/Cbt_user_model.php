...
class Cbt_user_model extends CI_Model{
     ...
     */ 
     ...
     function update_user_to_online($id){
        $this->db->where('user_id', $id);
        $this->db->update($this->table, array('is_logged_in' => 1));
    }

    function update_user_to_offline($id){
        $this->db->where('user_id', $id);
        $this->db->update($this->table, array('is_logged_in' => 0));
    }
