...
class Cbt_tes_soal_model extends CI_Model{
    ...
    function get_by_testsoal_soal($tesuser_id){
        $this->db->select("COALESCE(cbt_tes_soal_jawaban.soaljawaban_answer, '-') as soaljawaban_answer, cbt_tes_soal.tessoal_soal_id, cbt_tes_soal.tessoal_tesuser_id, cbt_tes_soal.tessoal_nilai, cbt_tes_soal.tessoal_jawaban_text")
                 ->where('cbt_tes_soal.tessoal_tesuser_id="'.$tesuser_id.'"')
                 ->join('cbt_tes_soal_jawaban', 'cbt_tes_soal_jawaban.soaljawaban_tessoal_id = cbt_tes_soal.tessoal_id AND cbt_tes_soal_jawaban.soaljawaban_selected = 1', "left")
                 ->from($this->table);
                //  ->order_by('cbt_tes_soal.tessoal_id', 'ASC');
        return $this->db->get();
    }
