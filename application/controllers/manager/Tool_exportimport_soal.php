...
class Tool_exportimport_soal extends Member_Controller {
    ...
    public function exportsoal(){
		...
		$this->zip->add_data('ttgcbt-soal.sql', $sql);

		$path = $this->config->item('upload_path');
		$this->zip->read_dir($path);

		$this->zip->download('ttgcbt-soal.zip');
    }
    ...
    public function exportsoal(){
		...
		$this->zip->add_data('ttgcbt-soal.sql', $backup);
    ...
    public function importsoal(){
        ...
        if(!empty($_FILES['userfile']['name'])){    
	        ...
	        if($this->cbt_topik_model->count_all()->row()->hasil>0){
	        	$data['error'] = 'Data Soal Gagal di Import. Masih ada Topik di ttg CBT. Silahkan hapus Topik terlebih dahulu.';
	        }else{
	            ...
	            if (!$this->upload->do_upload()){
			        ...
			        if ($zip->open($config['upload_path'].$upload_data['file_name']) === TRUE)			        {
			            ...
			            // Import SQL
			            if (file_exists($config['upload_path'].'import/ttgcbt-soal.sql')) {
			            	$isi_file = file_get_contents($config['upload_path'].'import/ttgcbt-soal.sql'); 
