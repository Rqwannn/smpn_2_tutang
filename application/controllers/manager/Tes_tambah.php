...
class Tes_tambah extends Member_Controller {
    ...
    function tambah_soal(){
        ...
        $this->form_validation->set_rules('soal-set', 'Set Soal','required');
        ...
        if($this->form_validation->run() == TRUE){
        	...
        	$data['tset_soal_set'] = $this->input->post('soal-set', true);
    ...
    function get_datatable_soal(){
	    ...
	    foreach ($query as $temp) {			
            ...
            $set_soal = " Set Soal [{$temp->tset_soal_set}]";
			...
			if(!empty($query_modul->modul_nama)){
				$record[] = 'Modul '.$query_modul->modul_nama.'<br />'.$query_topik->topik_nama.' ['.$temp->tset_jumlah.'] ['.$temp->tset_jawaban.']'.$ket_acak . $set_soal;
			}else{
