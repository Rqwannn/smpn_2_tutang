...
class Modul_soal extends Member_Controller {
	...
	function soal_from_another_topik(){
		$this->load->library('form_validation');
		$this->form_validation->set_rules('tambah-topik', 'Topik','required');

		if($this->form_validation->run() == TRUE){
			$id_topik = $this->input->post('tambah-topik', TRUE);
			$result = [];

			$data = $this->cbt_soal_model->get_data_by_topik($id_topik)->result();

			foreach($data as $row){
				$result[] = [
					"id" => $row->soal_id,
					"soal" => $row->soal_detail,
					"tipe_soal" => $row->soal_tipe,
				];
			}
	...
	function tambah_soal_by_topik(){
		$this->load->library('form_validation');
        
        $this->form_validation->set_rules('topik-id', 'Topik','required');
        $this->form_validation->set_rules('soal-id', 'Soal','required');
		$this->form_validation->set_rules('tambah-set-soal', 'Set Soal','required');

		if($this->form_validation->run() == TRUE){
			$id_soal = $this->input->post('soal-id', TRUE);
        	$id_topik = $this->input->post('topik-id', TRUE);
        	$set_soal = $this->input->post('tambah-set-soal', TRUE);

			$get_soal = $this->cbt_soal_model->get_data_by_soal_id($id_soal);
			unset($get_soal["soal_id"]);
			$get_soal["soal_topik_id"] = $id_topik;
			$get_soal["soal_set"] = $set_soal;

			$this->cbt_soal_model->save($get_soal);	
    ...
    function tambah(){
        ...
        if($this->form_validation->run() == TRUE){
        	...
        	$set_soal = $this->input->post('tambah-set-soal', FALSE);
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
			...
			$cek1 = $query_jawaban->hasil == $temp->jawaban_benar; 
			$cek2 = $query_jawaban->hasil == $temp->jawaban_salah; 

			if($cek1 || $cek2){
				if( ($query_jawaban->hasil == 0 && $cek1) || ($query_jawaban->hasil == 0 && $cek2)){
					$analisa = "-";
				} else {
					$analisa = "Tidak Layak";
				}
			...
			$jumlahSoalBenar = $temp->jawaban_benar;
			$totalSoal = $query_jawaban->hasil;

			if($jumlahSoalBenar == 0 && $temp->jawaban_salah > 0){
				$tingkatKesulitan = "Sulit";
			} else if($jumlahSoalBenar == 0 && $temp->jawaban_salah == 0){
				$tingkatKesulitan = "-";
			} else {
            ...
            $record[] = '<div style="text-align: center;">'.$temp->jawaban_benar.'</div>';
            $record[] = '<div style="text-align: center;">'.$temp->jawaban_salah.'</div>';
            $record[] = '<div style="text-align: center;">'.$tingkatKesulitan.'</div>';
            $record[] = '<div style="text-align: center;">'.$analisa.'</div>';
